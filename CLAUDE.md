...
비주얼(컬러/톤/이미지)은 그리온 고유 브랜드를 따르고, 아래는 **구조·패턴·구현 방식**에 대한 참고 기준. UI/UX 변경 시 아래 순서로 검토.

### 원칙 — Shopify Polaris

레이아웃, 리소스 목록, 폼, 빈 상태, 콘텐츠(마이크로카피) 원칙 참고.

- **레이아웃**: 연관된 정보는 카드/섹션으로 묶기 (근접성 원칙 — 가까울수록 연관돼 보임). 구분선은 표에만 사용, 그 외엔 크기/여백/명암으로 위계 표현
- **콘텐츠**: 불필요한 단어 제거, 동사로 시작하는 직접적인 문구 사용 ("앱 추가하기" O, "앱을 추가할 수 있습니다" X), 버튼 텍스트에 불필요한 관사/조사 금지, 한 화면엔 하나의 주요 과업만
- **빈 상태(Empty state)**: 이미지 + 제목 + 본문 + 기본 액션 버튼 1개 구조. 제목은 상태 설명이 아니라 혜택/다음 행동 중심 ("주문 없음" X → "주문을 만들고 인보이스를 보내보세요" O). 기본 액션은 반드시 1개만
- **리소스 목록**: 각 항목은 탭 가능해야 하고, 목록 상단에 종류/개수를 명시 (예: "쪽지 3개"). 상세 열람보단 빠른 스캔에 최적화

### 화면 패턴 — Medusa Admin (@medusajs/ui)

오픈소스 이커머스 어드민 대시보드. 공식 UX 가이드는 없어서 실제 화면 구조를 패턴 참고용으로만 사용.

- **정보 밀도**: 여백보다 기능 우선. 보조 정보는 작은 텍스트/뱃지로 압축
- **Drawer**: 상세 화면을 벗어나지 않고 편집/보조 액션을 처리할 때 사용 — 이 프로젝트에선 shadcn `Sheet`가 동일 역할
- **2단 상세 레이아웃**: 데스크톱 기준 메인 콘텐츠 + 사이드 메타정보 분리하는 패턴. 모바일 폭 고정인 그리온은 나란히 배치는 불가하니, "메인 콘텐츠 vs 메타/설정 정보"를 시각적으로 구분된 섹션으로 세로 스택하는 방식으로 응용

### 구현

- shadcn 컴포넌트/스킬(`shadcn`) 우선 활용
- 컴포넌트 합성 설계 시 `vercel-composition-patterns` 참고
- 성능/구조 판단 시 `vercel-react-best-practices` 참고

### 절대 바뀌지 않는 것

- **웹이지만 모바일 폭 고정 레이아웃** — 위 레퍼런스는 구조·패턴 참고용이지, 데스크톱 와이드 레이아웃을 가져오는 게 아님. `레이아웃 원칙` 섹션의 width 제한 규칙은 그대로 유지
