    ...
    >
      {MEMORY_PROMPTS.map((prompt, i) => (
        <button
          ...
          className="border-border bg-background hover:bg-background/70 flex w-44 shrink-0 cursor-pointer items-start rounded-xl border p-3 text-left text-sm leading-snug transition-colors"
        >
          {prompt}
        </button>
      ))}
